...
try:
    # see http://pythonhosted.org/PyMuPDF
    # for the PyMuPDF documentation including installation 
    import fitz
    mupdf = True
    ...
    try:
        ...
        msg = "PyMuPDF or PyPDF2 must be available to use this"
...
#----------------------------------------------------------------------------
    ...
    # that are only needed if using pyPDF2
    ...
    def LoadFile(self, pdf_file):
        ...
        :param `pdf_file`: can be either a string holding a filename path or
         a file-like object (pyPDF2 only?).
        """
...
class mupdfProcessor(object):
    ...
    def RenderPage(self, gc, pageno, scale=1.0):
        ...
        try:
            ...
            gc.DrawBitmap(gbmp, 0, 0, pix.width, pix.height)
    ...
    def ProcessOperators(self, opslist, pdf_fonts):
        ...
        for operand, operator in opslist :
            ...
            #    g.charSpacing = float(operand[0])
                ...
                drawlist.extend(self.DrawTextString(operand[0].original_bytes.decode('latin-1')))
        ...
        # a DrawBitmap operation into the operation itself - the width and height extracted
        # from the bitmap was the actual size of the PDF image not the size it is to be drawn
...
def _AsciiBase85DecodePYTHON(instring):
