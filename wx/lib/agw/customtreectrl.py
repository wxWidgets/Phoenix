...
class GenericTreeItem(object):
    ...
    def __init__(self, parent, text="", ct_type=0, wnd=None, image=-1, selImage=-1, data=None, separator=False, on_the_right=True):
        ...
        self._hasHilight = False    # True when item is selected. Same as focused
    ...
    def GetCurrentCheckedImage(self):
        ...
        if not self._type or self._type == 0:
            return None
...
#        event.Skip()
    ...
    def CalculateLevel(self, item, dc, level, x_colstart, y, align=0):
        ...
        :param `dc`: an instance of :class:`wx.DC`;
        :param integer `level`: the item level in the tree hierarchy;
        :param integer `x_colstart`: the starting column; 
...
if __name__ == '__main__':
    ...
    class MyFrame(wx.Frame):
        ...
        def __init__(self, parent):
            ...
            print("Tree populate took %.3fms" % elapsed)
        ...
        def OnShowButton(self, event, parent):
            ...
            self.tree.Thaw()
            elapsed = abs(time.time() - start) * 1000
            print("Added %d windows in %.3fms" % (len(self.tree._itemWithWindow) - count, elapsed))
        ...
        def OnActivated(self, event):
            item = event.GetItem()
            print("Item %r x=%r, y=%r, width=%r, height=%r" %
                  (item._text, item.GetX(), item.GetY(), item.GetWidth(), item.GetHeight()))
